#!/bin/bash
# key scheme: C + S + E
# faketime "2024-01-01 12:00:00" ./standalone_RSA-PSS_dual-key.sh
# OpenSSL 1.1.1 and above

test_commonName="Phillip Runaway"
...
### BEGIN SMIME standalone dual-key [RSA-PSS] ROOT x509v3_config
...
### END SMIME standalone dual-key [RSA-PSS] ROOT x509v3_config
EOF
...
### BEGIN SMIME standalone dual-key [RSA-PSS] USER x509v3_config
...
### END SMIME standalone dual-key [RSA-PSS] USER x509v3_config
EOF
...
openssl x509 -req -days "${user_usage_period_days}" -set_serial "0x$(custom_serial)" -in "private/${user_alias}/csr_user_S.csr" -CA "private/root/cert_root.crt" -CAkey "private/root/key_root.pem" -extfile "${OPENSSL_CONF}" -extensions x509_smime_rsa_user_S_ext -sigopt rsa_padding_mode:pss -"${default_md_user_S}" -sigopt rsa_mgf1_md:"${default_md_user_S}" -sigopt rsa_pss_saltlen:"${rsa_pss_saltlen_user_S}" > "private/${user_alias}/cert_user_S.crt"
openssl x509 -req -days "${user_usage_period_days}" -set_serial "0x$(custom_serial)" -in "private/${user_alias}/csr_user_E.csr" -CA "private/root/cert_root.crt" -CAkey "private/root/key_root.pem" -extfile "${OPENSSL_CONF}" -extensions x509_smime_rsa_user_E_ext -sigopt rsa_padding_mode:pss -"${default_md_user_E}" -sigopt rsa_mgf1_md:"${default_md_user_E}" -sigopt rsa_pss_saltlen:"${rsa_pss_saltlen_user_E}" > "private/${user_alias}/cert_user_E.crt"
...
#
# .   standalone_RSA-PSS_dual-key.sh
# |
# +---private
# |   +---root
# |   |       cert_root.crt
